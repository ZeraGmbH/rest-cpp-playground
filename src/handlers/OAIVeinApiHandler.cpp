...
OAIRpcResponse OAIVeinApiHandler::getRPCAnswer(OAIRpcRequest rpc_request)
{
    ...
    if(storage->hasStoredValue(rpc_request.getEntityId(), rpc_request.getRpcName())){
        QVariant value = storage->getStoredValue(rpc_request.getEntityId(), rpc_request.getRpcName());
        QString returnValue = variantToJsonString(value);
        if(returnValue != "null")
        {
    ...
    {
        response.setStatus(422);
        response.setReturnInformation("\"Timeout or not existing entity or component\"");
    }
    return response;
...
void OAIVeinApiHandler::apiV1VeinRpc1Post(OAIRpcRequest oai_rpc_request) {
    ...
    if( reqObj != nullptr )
    {
        ...
        std::shared_ptr<TaskTemplate> taskSharedPtr = m_veinEntry->rpcToVein(oai_rpc_request.getEntityId(), oai_rpc_request.getRpcName(), parametersMap);
        //std::shared_ptr<TaskTemplate> taskSharedPtr = std::move(task);
        auto conn = std::make_shared<QMetaObject::Connection>();
        *conn = connect(taskSharedPtr.get(), &TaskTemplate::sigFinish, this, [conn, reqObj, res, taskSharedPtr, oai_rpc_request, this](bool ok, int taskId){
            OAIRpcResponse res = getRPCAnswer(oai_rpc_request);
