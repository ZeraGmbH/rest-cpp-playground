...
std::shared_ptr<TaskTemplate> VeinEntry::rpcToVein(int entityId, QString rpc_name, QVariantMap parameters)
{
    if(m_storage.getDb()->hasEntity(entityId)) {
        ...
        connect(taskSharedPtr.get(), &TaskTemplate::sigFinish, this, [taskSharedPtr](bool ok, int taskId){
            //m_cmdEventHandlerSystem->removeItem(task);
        });
        return taskSharedPtr;
    }
    else
        return TaskSimpleVeinRPCInvoker::create(entityId, rpc_name, parameters, m_cmdEventHandlerSystem);
}
